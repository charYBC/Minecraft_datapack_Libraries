...
#execute as @a[tag=in_grind_stone] if items entity @s inventory.* *[minecraft:custom_data~{temp_remove:1}] at @s as @e[tag=copy_data] if score @s custom_enchantments_id = @p custom_enchantments_id run function custom_enchantments:grind_stone_functions/set_back
